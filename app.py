...
# Durée totale avec pauses, repos, chargement
...
# Coût unitaire dégressif (exponentiel)
def cout_unitaire_degressif(cout_total_complet, n_palette):
    k = 0.65
    return round((cout_total_complet * 33) / (n_palette ** k), 2)
...
if submitted:
    ...
    if dist is not None:
        ...
        - **Coût unitaire moyen (pour {nb_palettes_form} palettes)** : {cout_palette} €
        """)

        # Tableau de dégressivité
        st.subheader("📊 Coût unitaire par palette selon le nombre transporté (dégressivité)")
        data = {
            "Nombre de palettes": list(range(1, 34)),
            "Coût unitaire (€)": [cout_unitaire_degressif(cout_total, n) for n in range(1, 34)]
        }
